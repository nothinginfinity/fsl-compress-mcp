...
async function handle(name,args,env,ctx){
  ...
  if (name === "query_compressed") {
    ...
    const branch=args.branch||'main',repoKey=owner+'/'+repo,termLower=(term||'').toLowerCase();const limit=Math.min(args.limit||20,50);const rows=await dbAll(env.DB,'SELECT chunk_id,file_path,chunk_type,top_keywords,orig_bytes FROM fsl_chunks WHERE repo_key=? AND branch=? AND (lower(top_keywords) LIKE ? OR lower(file_path) LIKE ?) LIMIT ?',[repoKey,branch,'%'+termLower+'%','%'+termLower+'%',limit]);return{ok:true,repo:repoKey,branch,term:termLower,count:rows.length,matches:rows};
  }
  ...
  if (name === "compression_stats") {
    ...
    const branch=args.branch||'main',repoKey=owner+'/'+repo;const rows=await dbAll(env.DB,'SELECT * FROM fsl_jobs WHERE repo_key=? AND branch=? ORDER BY created_at DESC LIMIT 1',[repoKey,branch]);if(!rows.length)return{ok:false,error:'no job found for this repo/branch'};return{ok:true,job:rows[0]};
  }
